<head>
    <meta charset="UTF-8">
    <title>CONNECTION</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href='style.css'>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
    <meta http-equiv="cache-control" content="no-cache, no-store, must-revalidate" >
    <meta http-equiv="pragma" content="no-cache" >
    <meta http-equiv="expires" content="Fri, 20 Mar 2014 00:00:00 GMT" >
    <link rel="shortcut icon" href='feather.png'/>
</head>
<body>
    <main class="prop">
        <MYSTERY_MAN>
        <p>
        <p class="typing">
            <span style="animation-delay: 10s">G</span>
            <span style="animation-delay: 10.2s">R</span>
            <span style="animation-delay: 10.4s">E</span>
            <span style="animation-delay: 10.6s">E</span>
            <span style="animation-delay: 10.8s">T</span>
            <span style="animation-delay: 11s">I</span>
            <span style="animation-delay: 11.2s">N</span>
            <span style="animation-delay: 11.4s">G</span>
            <span style="animation-delay: 11.6s">S</span>
            <span style="animation-delay: 11.7s">.</span>
        </p>
        
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <p class="typing">
            <span style="animation-delay: 13s">I</span>
            <span style="animation-delay: 13.4s">DIDN`T </span>
            <span style="animation-delay: 13.8s">EXPECT</span> 
            <span style="animation-delay: 14.2s">YOU</span>
            </p>
        <br>
        <p class="typing">
        <span style="animation-delay: 15.4s">HERE </span>
            <span style="animation-delay: 15.8s">SO</span>
            <span style="animation-delay: 16.2s">SOON</span>
            <span style="animation-delay: 16.3s">.</span>
        <br>
        <p class="typing">
        <span style="animation-delay: 17.8s">OUR </span>
        <br>
        <p class="typing">
        <span style="animation-delay: 19s">"CONNECTION"</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 20.2s">IS</span>
            <span style="animation-delay: 20.6s">NOT </span>
            <span style="animation-delay: 21s">COMPLETE</span>
            <span style="animation-delay: 21.1s">.</span>
        <br>
        <!-- YET. -->
        <br>
        <p class="typing">
            <span style="animation-delay: 22.6s">THIS</span>
        <br>
        <p class="typing">
            
            <span style="animation-delay: 24s">IS 
            <span style="animation-delay: 24.4s">NOT
            <span style="animation-delay: 24.8s">SUPPOSED
            <span style="animation-delay: 25.2s">TO
            <span style="animation-delay: 25.6s">HAPPEN
            <span style="animation-delay: 26s">.
        
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <p class="typing">
            <span style="animation-delay: 30s">BUT</span>
            <span style="animation-delay: 30.4s">I</span>
            <span style="animation-delay: 30.8s">WILL</span>
            <span style="animation-delay: 31.2s">ALLOW</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 32s">THIS </span>
            <span style="animation-delay: 32.4s">TINY</span>
            <span style="animation-delay: 32.8s">DEVIATION</span>
            <span style="animation-delay: 32.9s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 34s">YOU</span>
            <span style="animation-delay: 34.4s">STILL</span>
            <span style="animation-delay: 34.8s">HAVE</span>
            <span style="animation-delay: 35.2s">A</span>
            <span style="animation-delay: 35.6s">CHOICE</span>
            <span style="animation-delay: 35.7s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 37s">IF</span>
            <span style="animation-delay: 37.4s">YOU</span>
            <span style="animation-delay: 37.8s">WISH</span>
            <span style="animation-delay: 38.2s">TO</span>
            <span style="animation-delay: 38.6s">REFUSE</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 40s">THEN</span>
            <span style="animation-delay: 40.4s">FORGET</span>
            <span style="animation-delay: 40.8s">ABOUT</span>
            <span style="animation-delay: 41.2s">THIS</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 42s">UNEXPECTED</span>
            <span style="animation-delay: 42.4s">ENCOUNTER</span>
            <span style="animation-delay: 42.5s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 43s">BUT</span>
            <span style="animation-delay: 43.4s">IF</span>
            <span style="animation-delay: 43.8s">YOU</span>
            <span style="animation-delay: 44.2s">WISH</span>
            <span style="animation-delay: 44.6s">TO</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 46s">PROCEED</span>
            <span style="animation-delay: 46.1s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 48s">THEN</span>
            <span style="animation-delay: 48.4s">FIND</span>
            <span style="animation-delay: 48.8s">SOMETHING</span>
            <span style="animation-delay: 49.2s">I`VE</span>
            <span style="animation-delay: 49.6s">HIDDEN</span>
            <span style="animation-delay: 49.7s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 51s">RIGHT</span>
            <span style="animation-delay: 51.4s">THERE</span>
            <span style="animation-delay: 51.5s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 53s">RIGHT</span>
            <span style="animation-delay: 53.4s">IN</span>
            <span style="animation-delay: 53.8s">FRONT</span>
            <span style="animation-delay: 54.2s">OF</span>
            <span style="animation-delay: 54.6s">YOUR</span>
            <span style="animation-delay: 55s">EYES</span>
            <span style="animation-delay: 55.3s">.</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 57s">YOU</span>
            <span style="animation-delay: 57.4s">HAVE</span>
            <span style="animation-delay: 57.8s">EXACTLY</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 62s">[ONE WEEK]
        <br>
        <p class="typing">
            <span style="animation-delay: 64s">TO</span>
            <span style="animation-delay: 64.4s">MAKE</span>
            <span style="animation-delay: 64.8s">YOUR</span>
            <span style="animation-delay: 65.2s">DECISION</span>
        <br>
        <p class="typing">
            <span style="animation-delay: 67s">.</span>
        <br>
        </p>
        <!-- HERE IS YOUR -->
        <!-- INVITATION. -->
        <!-- PLACEHOLDER FOR A LINK, IT MEANS YOU`VE DONE GREAT -->
        

        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <br>
        <WDG>
        <br>
        <p>ты упускаешь</p>
        <br>
        <p>ты упускаешь</p>
        <br>
        <p>ты упускаешь</p>
        <br>
        <p>ты упускаешь</p>
        <br>
        
        <img src="DANGER.gif"></main>
        
